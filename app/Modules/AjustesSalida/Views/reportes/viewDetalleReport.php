...
<div class="border-1 p-3" id="contentExport">
        ...
        <tr>
            ...
            <td style="width:30%; text-align:center; vertical-align: middle; border: 1px solid #e9ecef">
                <div class="text-center">
                    <h5 class="fw-bold mb-2">AJUSTE DE SALIDA</h5>
                    <h6 class="text-danger mb-2">#<?= str_pad($ajuste->ajes_secuencial, 5, '0', STR_PAD_LEFT) ?></h6>
                    <p class="fw-bold small mb-1">Fecha de Emisión:<br><span class="text-primary"><?= date('d/m/Y', strtotime($ajuste->ajes_fecha)) ?></span></p>
                    <?php
                    $estado = [
                    ...
                    [$estadoClass, $estadoTexto] = $estado[$ajuste->ajes_estado] ?? ['bg-secondary', 'DESCONOCIDO'];
                    ?>
    ...
    <table border="1" style="width:100%; margin-bottom:15px;" class="table table-bordered">
        <tr>
            ...
            <td style="width:50%; vertical-align:top; padding: 10px">
                <p class="fw-bold border-bottom mb-2"><strong><i class="fas fa-building me-2"></i> Información del Cliente</strong></p>
                <br>
                <div class="bg-light p-2 rounded p-2">
                    <p class="mb-1"><strong>&nbsp;RUC:</strong> <?= esc($ajuste->clie_dni) ?></p>
                    <p class="mb-1"><strong>&nbsp;Proveedor:</strong> <?= esc($ajuste->clie_razon_social) ?></p>
                    <p class="mb-0"><strong>&nbsp;Observaciones:</strong> <?= esc($ajuste->ajes_observaciones ?: 'Sin observaciones') ?></p>
                </div>
            </td>
        </tr>
    </table>
        ...
        <table border="1" width="100%" cellspacing="0" class="table table-bordered">
            ...
            <tbody>
                ...
                foreach ($ajuste->detalle as $idx => $item):
                    $totalGeneral += $item->ajsd_itemcostoxcantidad;
                    $totalUnidades += $item->ajsd_itemcantidad;
                    ?>
                    <tr>
                        ...
                        <td class="text-center">
                            <span class="text-info">
                                <?= $item->ajsd_itemcantidad ?>
                            </span>
                        </td>
                        <td class="text-end small">$<?= number_format($item->ajsd_itemcosto, 2) ?></td>
                        <td class="text-end fw-semibold small">
                            $<?= number_format($item->ajsd_itemcostoxcantidad, 2) ?>
                        </td>
                    </tr>
                <?php endforeach; ?>
            </tbody>
