    ...
    <div class="card card-system card-outline">
        <div class="card-header d-flex justify-content-between align-items-center">
            <h5 class="card-title text-system">
                <i class="fas fa-chart-line me-2"></i> Dashboard de Ajustes de Salida
            </h5>
        ...
        <div class="card-body">
            ...
            <div class="row g-3 mb-3">
                <div class="col-xl-4">
                    <div class="dashboard-box">
                        <div id="chartEstadosSalidas" class="dashboard-chart"></div>
                    </div>
                </div>

                <div class="col-xl-4">
                    <div class="dashboard-box">
                        <div id="chartMotivosSalidas" class="dashboard-chart"></div>
                    </div>
                </div>

                <div class="col-xl-4">
                    <div class="dashboard-box">
                        <div id="chartBodegasSalidas" class="dashboard-chart"></div>
                    </div>
                </div>
            </div>
            ...
            <div class="row g-3">
                <div class="col-xl-8">
                    <div class="dashboard-box">
                        <div id="chartTendenciaSalidas" class="dashboard-chart"></div>
                    </div>
                </div>

                <div class="col-xl-4">
                    <div class="dashboard-box mb-3">
                        <div id="chartCentrosCostoSalidas" class="dashboard-chart"></div>
                    </div>
                    <div class="dashboard-box">
                        <div id="chartServiciosSalidas" class="dashboard-chart"></div>
                    </div>
                </div>
            </div>
        </div>
...
<script type="text/javascript">
    ...
    var dashboardServicios = <?= json_encode($dashboardServicios ?? []) ?>;
    ...
    if (window.appDashboardAjustesSalida) {
        window.appDashboardAjustesSalida.unmount();
    }

    window.appDashboardAjustesSalida = Vue.createApp({
        components: {
        ...
        methods: {
            ...
            async cargarDashboard() {
                ...
                try {
                    this.loading = true;
                    const {data} = await axios.post(this.url + '/ajustessalida/getDataDashboard', datos);
            ...
            renderCharts() {
                ...
                this.charts.estados = Highcharts.chart('chartEstadosSalidas', {
                    chart: {type: 'pie'},
                ...
                this.charts.motivos = Highcharts.chart('chartMotivosSalidas', {
                    colors: dashboardColors,
                    chart: {type: 'column'},
                    title: {text: 'Salidas por motivo'},
                    xAxis: {
                        categories: this.motivos.map(item => item.motivo || 'SIN MOTIVO')
                    },
                    yAxis: {title: {text: 'Valor de salida'}},
                    tooltip: {valuePrefix: '$'},
                    plotOptions: {
                        column: {
                            colorByPoint: true
                ...
                this.charts.bodegas = Highcharts.chart('chartBodegasSalidas', {
                    colors: dashboardColors,
                    chart: {type: 'bar'},
                    title: {text: 'Salidas por bodega'},
                    xAxis: {
                ...
                this.charts.tendencia = Highcharts.chart('chartTendenciaSalidas', {
                    chart: {type: 'areaspline'},
                    title: {text: 'Tendencia mensual de salidas'},
                    xAxis: {
                ...
                this.charts.centrosCosto = Highcharts.chart('chartCentrosCostoSalidas', {
                    colors: dashboardColors,
                    chart: {type: 'bar'},
                    title: {text: 'Salidas por centro de costo'},
                    xAxis: {
                ...
                this.charts.servicios = Highcharts.chart('chartServiciosSalidas', {
                    colors: dashboardColors,
                    chart: {type: 'column'},
                    title: {text: 'Salidas por servicio'},
                    xAxis: {
                        categories: this.servicios.map(item => item.servicio || 'SIN SERVICIO')
                    },
                    yAxis: {title: {text: 'Valor de salida'}},
                    tooltip: {valuePrefix: '$'},
                    plotOptions: {
                    ...
                    series: [{
                            name: 'Total',
                            data: this.servicios.map(item => Number(item.valor || 0))
                        }]
                });
            },
    ...
    window.appDashboardAjustesSalida.mount('#app');
</script>
