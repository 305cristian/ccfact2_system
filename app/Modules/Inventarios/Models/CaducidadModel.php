 ...
 * @time 3:34:19 p.m.
 */
class CaducidadModel extends \CodeIgniter\Model {

    //put your code here

    public function getProductosCaducidad(array $filtros, ?int $start = null, ?int $length = null, ?string $search = null, ?string $orderBy = null, ?string $orderDir = null): array {
        ...
        if (!empty($filtros['caducFechas'])) {
            $rangoFechas = explode(' a ', $filtros['caducFechas']);
            $fDesde = trim($rangoFechas[0]);
            $fHasta = isset($rangoFechas[1]) ? trim($rangoFechas[1]) : trim($rangoFechas[0]);
            $builder->where(['tb8.lot_fecha_caducidad <= ' => $fHasta, 'tb8.lot_fecha_caducidad >= ' => $fDesde]);
        }
