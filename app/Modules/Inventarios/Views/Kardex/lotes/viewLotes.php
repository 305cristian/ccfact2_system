    ...
    <div class="card card-system card-outline">
        <div class="card-header">
            <h5 class="card-title text-system">
                <i class="fas fa-box"></i> Kardex Por Producto y Lote
            </h5>           
        </div>
        ...
        <div class="card-body">
            <fieldset>
                ...
                <div class="row col-md-12">
                    ...
                    <div class="col-md-2 col-sm-6  form-group-custom">
                        <div class="d-flex justify-content-between align-items-center border">
                            <span class="input-group-text bg-cris-system">
                                <i class="fas fa-warehouse me-2"></i>Lotes
                            </span>
                            <vue-select
                                class="flex-grow-1"
                                :options="listaLotes"
                                label="lot_lote"
                                v-model="filtros.kardLoteId"
                                :reduce="l => l.id"
                                placeholder="Seleccione una lote"
                                @option:selected="onSelectLote()"/>
                        </div>
                    </div>
                ...
                <br>
                <div class="row col-md-12">
                    <span v-if='nombreProducto' class="badge badge-primary fs-6">{{nombreProducto}} <span class="badge bg-success fs-6">{{loteProducto}}</span></span>
                </div>
            </fieldset>
            <div  v-if="!showContent" class="empty-state">
                ...
                <p>No hay kardex por Lotes Generado</p>
            ...
            <div v-else>
                ...
                <div class="table-responsive mt-3">
                    <table class="table table-hover table-striped align-middle" id='tblKardexProducto'>
                        ...
                        <tbody>
                            <tr v-for="item in listaKardexLote" :key="item.id">
                            ...
                            <tr v-if="listaKardexLote.length === 0">
...
<script type="text/javascript">
    ...
    if (window.appKardexLote) {
        window.appKardexLote.unmount();
    }
    window.appKardexLote = Vue.createApp({
        components: {
        ...
        computed: {
            ...
            totalEntradas() {
                return this.listaKardexLote.reduce((acc, item) => acc + parseFloat(item.entrada || 0), 0).toFixed(2);
            },

            totalSalidas() {
                return this.listaKardexLote.reduce((acc, item) => acc + parseFloat(item.salida || 0), 0).toFixed(2);
            },

            saldoFinal() {
                if (this.listaKardexLote.length === 0)
                    return 0;
                return parseFloat(this.listaKardexLote[this.listaKardexLote.length - 1].kardex_total).toFixed(2);
            }
        },
        methods: {
            ...
            async searchDataReport() {
                ...
                if (!this.filtros.kardLoteId) {
                    sweet_msg_toast('warning', 'Debe seleccionar un lote para poder consultar el kardex');
                    return;
                }
                ...
                try {
                    ...
                    const {data} = await axios.post(this.url + "/kardex/getKardexLote", datos);

                    if (data.status === 'success') {
                        this.showContent = true;
                        this.listaKardexLote = data.data;
                        Swal.close();
            ...
            searchProductos(dataSerach) {
                ...
                this.searchTimeout = setTimeout(async () => {
                    try {
                        const {data} = await axios.post(this.url + '/comun/productos/searchProductosFull', datos);
                        if (data !== false) {
            ...
            async onSelectProducto(option) {
                ...
                await this.getLotesProducto(this.filtros.kardProductoId);
            },

            onSelectLote() {

                this.showContent = false;
                this.loteProducto = this.listaLotes.find(val => val.id === this.filtros.kardLoteId)?.lote || null;

            },

            async getLotesProducto(productoId) {
                const {data} = await axios.get(`${this.url}/comun/lotes/SearchsController/${productoId}`);
                if (data) {
    ...
    window.appKardexLote.use(AllDirectives);
    window.appKardexLote.mount('#app');
