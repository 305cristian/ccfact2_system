...
<div class="border-1 p-3" id="contentExport">
        ...
        <tr>
            ...
            <td style="width:30%; text-align:center; vertical-align: middle; border: 1px solid #e9ecef">
                <div class="text-center">
                    <h5 class="fw-bold mb-2">TRANSFERENCIA</h5>
                    <h6 class="text-danger mb-2">#<?= str_pad($transferencia->trb_secuencial, 5, '0', STR_PAD_LEFT) ?></h6>
                    <p class="fw-bold small mb-1">Fecha de Emisión:<br><span class="text-primary"><?= date('d/m/Y', strtotime($transferencia->trb_fecha)) ?></span></p>
                    <?php
                    $estado = [
    ...
    <br>
    <!-- Información de la transferencia y usuario a confirmar -->
    <table border="1" style="width:100%; margin-bottom:15px;" class="table table-bordered">
        <tr>
            <!-- Columna izquierda: origen -->
            <td style="width:50%; vertical-align:top; padding: 10px">
                <p class="fw-bold border-bottom mb-2"><strong><i class="fas fa-building me-2"></i> Información del Origen</strong></p>
                <br>
                <div class="bg-light p-2 rounded p-2">
                    <p class="mb-1"><strong>&nbsp;Bodega:</strong> <?= esc($transferencia->bod_nombre_origen) ?></p>
                    <p class="mb-0"><strong>&nbsp;Usuario:</strong> <?= esc($transferencia->user_create) ?></p>
                    <p class="mb-0"><strong>&nbsp;Fecha Emisión:</strong> <?= esc(date('d-m-Y', strtotime($transferencia->trb_fecha))) ?></p>
                    <p class="mb-0"><strong>&nbsp;Observaciones:</strong> <?= esc($transferencia->trb_observaciones ?: 'Sin observaciones') ?></p>
                </div>
            </td>

            <!-- Columna derecha: destino -->
            <td style="width:50%; vertical-align:top; padding: 10px">
                <p class="fw-bold border-bottom mb-2"><strong><i class="fas fa-building me-2"></i> Información del Destino</strong></p>
                <br>
                <div class="bg-light p-2 rounded p-2">
                    <p class="mb-1"><strong>&nbsp;Bodega:</strong> <?= esc($transferencia->bod_nombre_destino) ?></p>
                    <p class="mb-1"><strong>&nbsp;Usuario:</strong> <?= esc($transferencia->user_confirm) ?></p>
                    <p class="mb-1"><strong>&nbsp;Fecha de Confirmación:</strong> <?= esc(isset($transferencia->trb_fecha_confirmacion)? date('d-m-Y', strtotime($transferencia->trb_fecha_confirmacion)):'SIN CONFIRMAR') ?></p>
                </div>
            </td>
        </tr>
    </table>
    ...
    <div class="table-responsive">
        ...
        <table border="1" width="100%" cellspacing="0" class="table table-bordered">
            ...
            <tbody>
                ...
                $totalGeneral = 0;
                $totalUnidades = 0;
                foreach ($transferencia->detalle as $idx => $item):
                    $totalGeneral += $item->trbd_itemcostoxcantidad;
                    $totalUnidades += $item->trbd_itemcantidad;
                    ?>
                    <tr>
                        ...
                        <td class="text-center">
                            <span class="text-info">
                                <?= $item->trbd_itemcantidad ?>
                            </span>
                        </td>
                        <td class="text-end small">$<?= number_format($item->trbd_itemcosto, 2) ?></td>
                        <td class="text-end fw-semibold small">
                            $<?= number_format($item->trbd_itemcostoxcantidad, 2) ?>
                        </td>
                    </tr>
                <?php endforeach; ?>
            </tbody>
        ...
        <tr>
            ...
            <td style="width: 60%; vertical-align: middle; background-color: #d1ecf1; border-radius: 6px; padding: 8px 12px; border-radius: 10px">
                <div style="color: #0c5460;">
                    <p><strong>Total de Ítems:</strong> <?= count($transferencia->detalle) ?> productos</p>
